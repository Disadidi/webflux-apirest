...
public class ProductoController {
	...
	public Mono<ResponseEntity<Producto>> crearConFoto(Producto producto, @RequestPart FilePart file){
		...
		return file.transferTo(new File(path + producto.getFoto()))
				.then(service.save(producto)).map(p -> ResponseEntity.created(
...
//	}
	...
	public Mono<ResponseEntity<Producto>> ver(@PathVariable String id){
		/**
		 * service.findById -> retourne un Mono<Producto> il faut le modifier
		 * en Mono<ResponseEntity<Producto>> 
		 */
	...
	public Mono<ResponseEntity<Producto>> crear(@RequestBody Producto producto){
		...
		return service.save(producto).map(p -> ResponseEntity.created(
				URI.create("/api/productos/".concat(p.getId())))
				.contentType(MediaType.APPLICATION_JSON)
				.body(p));
	}
